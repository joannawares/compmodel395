---
title: "Herd Immunity Final Percentage of Infections Part 2"
...
Now let's change the initial conditions so that (half-1) of the total population (1 million) is suseptible and half is removed and only 1 is infected and see what happens. 
...
```{r, warning = FALSE}
times <- seq(0, 100, by = 1/2^4)
as.data.frame(SIROut) %>% ggplot() + geom_line(aes(x=time, y=I), color="red", size=1) + geom_line(aes(x=time, y=S), color="blue", size=1) + geom_line(aes(x=time, y=R), color="green", size=1) + ylim(0,1000000)
```
...
herd_infected_index <- which.max(as.data.frame(SIROut)$S < 400000)
herd_infected_index


herd_infected1 <- as.data.frame(SIROut)$I[herd_infected_index]
herd_infected1

print(which(as.data.frame(SIROut)$I == max(as.data.frame(SIROut)$I)))
...
```{r, warning = FALSE}
herd_day <- herd_infected_index/2^4 
herd_day
```
...
```{r, warning = FALSE}
print(as.data.frame(SIROut)$R[herd_infected_index])
```
...
```{r, warning = FALSE}
final_infected1 <- tail(as.data.frame(SIROut)$R,1)
final_infected1
```
...
final_infected2 <- tail(as.data.frame(SIROut)$R,1)
final_infected2
